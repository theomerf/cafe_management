...
type MonthlyIncomeGraphProps = {
    monthlyIncomeStats: OrderStats | undefined
    isLoading: boolean
    error: Error | null
}

export default function MonthlyIncomeGraph({ monthlyIncomeStats, isLoading, error }: MonthlyIncomeGraphProps) {
    ...
    useEffect(() => {
        if (!monthlyIncomeStats) return

        const labels = monthlyIncomeStats.labels.map((label) => label.toString());
        const incomes = monthlyIncomeStats.totalIncomes.map((income) => income);
                ...
                <div className="p-4 md:p-6">
                    <div className={`flex items-center mb-3 md:mb-4 ${isMobile ? "flex-col gap-2" : "flex-row justify-between"}`}>
                        <h2 className={`font-semibold text-violet-400 ${isMobile ? "text-lg text-center" : "text-2xl"}`}>
                            {currentYear} - Aylık Gelir Grafiği
                        </h2>
                    </div>
                    ...
                    <div className={`mt-3 md:mt-4 grid gap-2 md:gap-4 text-center ${isMobile ? "grid-cols-2" : "grid-cols-3"}`}>
                        ...
                        <div className="bg-green-50 rounded-lg p-3">
                            <p className="text-gray-600 text-sm">Ortalama/Ay</p>
