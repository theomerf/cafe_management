...
type HourlyIncomeDensityGraphProps = {
    hourlyDensityStats: OrderStats | undefined
    isLoading: boolean
    error: Error | null
}

export default function HourlyIncomeDensityGraph({ hourlyDensityStats, isLoading, error }: HourlyIncomeDensityGraphProps) {
    ...
    useEffect(() => {
        ...
        const incomes = hourlyDensityStats.totalIncomes.map((income) => income);

        const line: LineData = {
            id: "Saatlik Gelir Yoğunluğu",
            color: "#3b82f6",
                ...
                <div className="p-4 md:p-6">
                    <div className={`flex items-center mb-3 md:mb-4 ${isMobile ? "flex-col gap-2" : "flex-row justify-between"}`}>
                        <h2 className={`font-semibold text-violet-400 ${isMobile ? "text-lg text-center" : "text-2xl"}`}>
                            {currentYear} - Saatlik Gelir Yoğunluğu Grafiği
                        </h2>
                    ...
                    <div className={`mt-3 md:mt-4 grid gap-2 md:gap-4 text-center ${isMobile ? "grid-cols-2" : "grid-cols-3"}`}>
                        <div className="bg-blue-50 rounded-lg p-3">
                            <p className="text-gray-600 text-sm">Toplam Saatlik Gelir</p>
